...
# The 'punctuation_all' is the combination set of Chinese and English
# punctuation.
punctuation_zh = " 、，。°？！：；“”＂’‘～…【】［］（）《》｛｝×―－—·→℃%\t"
...
def synonym_cut(sentence, pattern="wf"):
    ...
    synonym_vector = []
    if pattern == "w":
        synonym_vector = [item for item in jieba.cut(
            sentence) if item not in filter_characters]
    elif pattern == "k":
...
def jaccard_basic(synonym_vector1, synonym_vector2):
    ...
    """
    count_intersection = list(
        set(synonym_vector1).intersection(
            set(synonym_vector2)))
...
def get_location(sentence):
    ...
    for word, tag in sv_sentence:
        if tag.startswith("Di02") or tag.startswith(
                "Di03") or tag == "Cb25A11#":
