...
"""
import copy
from .apilib import get_location_by_ip
...
class Robot(object):
    ...
    def init_user(self, userid=None, key=None):
        self.user = self.selector.select("User").where(
            "_.userid='" + userid + "'", "_.key='" + key + "'").first()
        if self.user:
    ...
    # def extract_navigation(self, question):
        ...
        # context="", tid="", ftid="", url="", behavior=0, parameter="",
        # txt="", img="", button="", valid=1)
    ...
    def extract_keysentence(self, question, data=None, threshold=0.40):
        ...
        if data:
            ...
            subgraph = [item['n']
                        for item in self.graph.run(match_string).data()]
        if subgraph:
            # 选取第一个匹配节点
            print("Similarity Score: Key sentence")
            # return self.extract_synonym(question, subgraph,
            # threshold=threshold)
            node = subgraph[0]
            return self.update_result(question, node)
        return self.update_result(question)
    ...
    def extract_keysentence_first(self, question, data=None, threshold=0.40):
        ...
        if subgraph:
            # 选取得分最高的
            print("Similarity Score: Key sentence")
            return self.extract_synonym_first(
                question, subgraph, threshold=threshold)
        return self.update_result(question)
    ...
    def remove_name(self, question):
        ...
        for robotname in ["小民", "小明", "小名", "晓明"]:
            if question.startswith(robotname) and len(
                    question) >= 4 and "在线" not in question:
    ...
    def search(self, question="question", tid="", userid="A0001", key="A0001"):
        ...
        error_page = dict(
            ...
            ftid="",
            url="",
            # Modify：场景内 behavior 统一为 0x1500。(2018-1-8)
            behavior=int("0x1500", 16),
            parameter="",
            txt="",
            img="",
            button="",
        ...
        for item in self.cmd_end_scene:
            if item == question:  # 完全匹配退出模式
                result['behavior'] = 0
                result['name'] = "退出"
                # Modify 2018-3-6 退出时返回的场景标签为当前场景
        ...
        if self.is_scene:  # 在场景中：语义模式+关键句模式
            ...
            if self.last_step_error:
                if question in self.yes:
                    ...
                    return result
            # 场景内所有节点
            match_scene = "MATCH (n:NluCell) WHERE n.topic='" + \
                self.topic + "' RETURN n"
            ...
            subscene_nodes = [item['n'] for item in scene_nodes if item['n']['ftid'] ==
                              self.amemory[-1]['tid'] or item['n']['ftid'] == self.amemory[-1]['ftid']]
            ...
            usergraph_all = [item['n']
                             for item in self.graph.run(match_graph).data()]
            if usergraph_all:
                # 同义句匹配 TODO：阈值可配置
                result = self.extract_synonym(
                    question, usergraph_all, threshold=0.90)
                # 关键词匹配 TODO：配置开关
                if not result["context"]:
