    ...
    def _time_me(func):
        ...
        def _wrapper(*args, **kwargs):
            ...
            if format_string == "s":
                ...
                print("%s %s %s" %
                      (func.__name__, info, 1000 * (end - start)), "ms")
            return result
        return _wrapper
    return _time_me
...
def get_current_time(format_string="%Y-%m-%d-%H-%M-%S", info=None):
    ...
    """
    assert isinstance(
        format_string, str), "The format_string must be a string."
    # Python3
    ...
    try:
        ...
        result = time.strftime(format_string.encode(
            'unicode-escape').decode(), time.localtime())
...
def write_excel(filename="demo.xlsx", sheets=None):
    ...
    for sheet in sheets:
        new_sheet = file.add_sheet(
            sheet["name"], cell_overwrite_ok=True)  # 创建sheet
        info = sheet["info"]
        ...
        for col, key in enumerate(info):
            new_sheet.write(
                0, col, key[1], set_excel_style(
                    'Arial Black', 220, True))
            new_sheet.write(
                1, col, key[0], set_excel_style(
                    'Arial Black', 220, True))
        for index, item in enumerate(sheet["items"]):
            for col, key in enumerate(info):
