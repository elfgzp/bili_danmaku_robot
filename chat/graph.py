...
class Database(object):
    ...
    def delete(self, pattern="n", label=None):
        ...
        if pattern == "all":
            ...
            self.graph.run(
                "MATCH (n)<-[r:" + label + "]-(m) DETACH DELETE r, n")
        elif pattern == "rm":
            self.graph.run(
                "MATCH (n)-[r:" + label + "]->(m) DETACH DELETE r, m")
        elif pattern == "nrm":
            self.graph.run(
                "MATCH (n)-[r:" + label + "]-(m) DETACH DELETE r, n, m")
    ...
    def add_nlucell(
        ...
        for question in name.split(delimiter):
            ...
            if question:  # 问题不能为空
                ...
                node = self.selector.select("NluCell").where(
                    "_.name ='" + question + "'",
                    "_.topic ='" + topic + "'",
                    "_.tid =" + match_tid).first()
                if node:  # 存在则覆盖
                    # node['name'] = question
    ...
    def handle_excel(self, filename=None, custom_sheets=[]):
        ...
        for sheet_name in sheet_names:
            ...
            config_node = self.selector.select(
                "Config", name=sheet_name).first()
            if not config_node:
                # 默认 self.user 已存在
                self.graph.run(
    ...
    def generate_testcases(
        ...
        new_sheet = file.add_sheet(
            "NluTest", cell_overwrite_ok=True)  # 创建sheet
        keys = ["问题", "答案", "是否通过", "改进建议"]
        # 生成表头
        new_sheet.write(
            0, 0, "本地语义常见命令问答测试", set_excel_style(
                'Arial Black', 220, True))
        for col, key in enumerate(keys):
            new_sheet.write(
                1, col, key, set_excel_style(
                    'Arial Black', 220, True))
        count = 0
        testlist = []
        # 生成内容
